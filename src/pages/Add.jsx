...
const onSubmitHandler = async (e) => {
  ...
  try {
    ...
    const response = await axios.post(backendUrl + '/api/product/add', formData, {
      headers: {
        'Authorization': `Bearer ${token}`,//Tells the server who you are and that you're authenticated
    ...
    <label htmlFor="image4">
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        <div> 
          ...
          <select onChange={(e)=>setCategory(e.target.value)} value={category} className=' px-3 py-2' required>
        ...
        <div>
          ...
          <select onChange={(e)=>setSubCategory(e.target.value)} value={subCategory} className=' px-3 py-2'  required>
        ...
        <div>
          <p>Product Price</p>
          <input onChange={(e)=>setPrice(e.target.value)} value={price} type="number" placeholder='0' required />
        </div>
       ...
       <div className='flex gap-2'>
        ...
        <button 
          className={`w-28 text-white px-4 py-2 rounded ${loading ? 'bg-gray-500 cursor-not-allowed' : 'bg-black hover:bg-gray-800'}`} 
          type='submit' 
          disabled={loading}
        >
          {loading ? 'Adding...' : 'Add'}
